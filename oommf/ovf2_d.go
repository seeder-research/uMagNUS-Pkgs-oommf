	...
	for iz := 0; iz < size[Z]; iz++ {
		for iy := 0; iy < size[Y]; iy++ {
			for ix := 0; ix < size[X]; ix++ {
				for c := 0; c < ncomp; c++ {
					tempNum := float32(dataArr[c][iz][iy][ix])
...
func writeOVF2DataBinary8(out io.Writer, array *data.Slice) {
	...
	for iz := 0; iz < size[Z]; iz++ {
		for iy := 0; iy < size[Y]; iy++ {
			for ix := 0; ix < size[X]; ix++ {
				for c := 0; c < ncomp; c++ {
					bytes = (*[8]byte)(unsafe.Pointer(&dataArr[c][iz][iy][ix]))[:]
					out.Write(bytes)
				}
			}
		}
...
func readOVF2DataBinary4(in io.Reader, array *data.Slice) {
	...
	for iz := 0; iz < size[Z]; iz++ {
		for iy := 0; iy < size[Y]; iy++ {
			for ix := 0; ix < size[X]; ix++ {
				for c := 0; c < ncomp; c++ {
					dataArr[c][iz][iy][ix] = data.DataType(readFloat32(in))
				}
			}
		}
	}
...
func readOVF2DataBinary8(in io.Reader, array *data.Slice) {
	...
	for iz := 0; iz < size[Z]; iz++ {
		for iy := 0; iy < size[Y]; iy++ {
			for ix := 0; ix < size[X]; ix++ {
				for c := 0; c < ncomp; c++ {
					dataArr[c][iz][iy][ix] = data.DataType(readFloat64(in))
				}
			}
		}
	}
